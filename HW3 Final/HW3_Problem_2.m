...
for  i = 1:100
    ...
    [nnz,weights, results] = growth_rate_output(inputs, growth);
...
for i = 1: size(data,1)
    ...
    sample = data(i,2:end);
    for j = 1:100
       predictedY(j) = horzcat(ones(1,1),sample)*(weightMatrix(j,:)'); %this generates a prediction based on the weights
    end
    %disp(predictedY);
